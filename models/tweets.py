...
class Tweets(Model):
    ...
    only = fields.CharField(max_length=255, description="唯一", unique=True)
    ...
    link = fields.CharField(max_length=255, description="推文链接")
    text = fields.TextField(description="推文内容")
    user = fields.JSONField(description="用户详情")
    date = fields.CharField(max_length=255, description="发布时间")
    external_link = fields.CharField(max_length=255, description="外部链接")
    quoted_post = fields.JSONField(max_length=255, description="引用的帖子")
